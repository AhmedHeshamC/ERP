...
let CacheInterceptor = (() => {
    ...
    var CacheInterceptor = _classThis = class {
        constructor(cacheService, reflector) {
            ...
            this.logger = new common_1.Logger(CacheInterceptor.name);
        }
        async intercept(context, next) {
            const cacheMetadata = this.reflector.get(cache_decorator_1.CACHE_KEY_METADATA, context.getHandler());
            const invalidateMetadata = this.reflector.get(cache_decorator_1.CACHE_INVALIDATE_METADATA, context.getHandler());
            ...
            const options = cacheMetadata.options || {};
         ...
         * Handle cache invalidation
         */
        async handleInvalidation(invalidateMetadata) {
