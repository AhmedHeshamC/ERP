...
/**
 * Error Tracking Service - Following SOLID Principles
 *
 * Single Responsibility: Only handles error tracking, analysis, and management
 * Open/Closed: Open for extension with new error types and analysis methods
 * Interface Segregation: Focused error tracking interface
 * Dependency Inversion: Depends on abstractions (ConfigService, EnhancedLoggerService)
 * Liskov Substitution: Simple, focused implementation
 *
 * KISS Principle: Simple, clear error tracking logic
 * No unnecessary complexity, focused on error analysis and management
 */
@Injectable()
export class ErrorTrackingService implements OnModuleInit, OnModuleDestroy {
   ...
   * KISS: Simple but effective fingerprinting algorithm
   */
  generateFingerprint(error: Error, context?: Partial<ErrorContext>, _metadata?: Record<string, any>): string {
  ...
  /**
   * Normalize error messages by removing variable values
   * KISS: Simple normalization patterns for common error variations
   */
  ...
  /**
   * Capture and process errors with full context
   * Single Responsibility: Only handles error capture and initial processing
   */
  captureError(
    error: Error | string,
  ...
  /**
   * Determine error severity based on characteristics
   * KISS: Simple severity classification rules
   */
  determineSeverity(
    error: Error,
    context?: Partial<ErrorContext>,
  ...
  private processError(errorEvent: ErrorEvent): void {
    ...
    this.enhancedLogger.error(
      ...
      errorEvent.name === 'UnknownError' ? new Error(errorEvent.message) : errorEvent,
      'ERROR_TRACKING',
      {
        errorId: errorEvent.id,
        fingerprint: this.generateFingerprint(
          errorEvent.name === 'UnknownError' ? new Error(errorEvent.message) : new Error(errorEvent.name),
          errorEvent.context,
          errorEvent.metadata
        ),
  ...
  private checkAlertConditions(_errorEvent: ErrorEvent): void {
    ...
    if (recentErrors.length > 50) {
      ...
      if (!this.isAlertCooldownActive(alertKey)) {
        this.enhancedLogger.error(
          `Error alert triggered: High error rate detected`,
          new Error(`Error rate: ${recentErrors.length} errors in 5 minutes`),
          'ERROR_ALERT',
          {
            alertId: alertKey,
    ...
    if (criticalErrors.length > 5) {
      ...
      if (!this.isAlertCooldownActive(alertKey)) {
        this.enhancedLogger.error(
          `Error alert triggered: Multiple critical errors detected`,
          new Error(`${criticalErrors.length} critical errors in 5 minutes`),
          'ERROR_ALERT',
          {
            alertId: alertKey,
  ...
  private isAlertCooldownActive(alertKey: string): boolean {
    const cooldownEnd = this.alertCooldowns.get(alertKey);
    if (!cooldownEnd) return false;
    return new Date() < cooldownEnd;
  }

  private setAlertCooldown(alertKey: string, durationMinutes: number = 5): void {
    this.alertCooldowns.set(alertKey, new Date(Date.now() + durationMinutes * 60 * 1000));
  }
  ...
  private generateErrorTrends(hours: number): ErrorTrend[] {
    ...
    for (let i = 0; i < hours; i += interval) {
      ...
      const uniqueErrors = new Set(periodErrors.map(e =>
        this.generateFingerprint(
          e.name === 'UnknownError' ? new Error(e.message) : new Error(e.name),
          e.context,
          e.metadata
        )
      )).size;
  ...
  private getTopErrorsForPeriod(errors: ErrorEvent[]): Array<{ fingerprint: string; count: number; name: string }> {
    ...
    errors.forEach(error => {
      const fingerprint = this.generateFingerprint(
        error.name === 'UnknownError' ? new Error(error.message) : new Error(error.name),
        error.context,
        error.metadata
      );
  ...
  private initializeErrorPatterns(): void {
    ...
    this.errorPatterns.set("cannot read property '([^']+)'", 'cannot_read_property');
    this.errorPatterns.set("cannot access property '([^']+)'", 'cannot_access_property');
    this.errorPatterns.set('is not a function', 'not_a_function');
    this.errorPatterns.set('is not defined', 'not_defined');
    this.errorPatterns.set('connection refused', 'connection_refused');
    this.errorPatterns.set('connection timeout', 'connection_timeout');
    this.errorPatterns.set('duplicate key', 'duplicate_key');
    this.errorPatterns.set('foreign key constraint', 'foreign_key_violation');
    this.errorPatterns.set('division by zero', 'division_by_zero');
  }
  ...
  private cleanupOldErrors(): void {
    ...
    const now = new Date();
    for (const [key, endTime] of this.alertCooldowns.entries()) {
      if (now >= endTime) {
