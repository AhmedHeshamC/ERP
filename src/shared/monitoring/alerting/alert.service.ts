...
export class AlertService implements OnModuleInit, OnModuleDestroy {
  ...
  async checkPerformanceAlerts(): Promise<void> {
    const endpoints = this.performanceService.getAllEndpointStats(); // Get all endpoint stats
    ...
    const avgResponseTime = endpoints.reduce((sum, ep) => sum + ep.averageResponseTime, 0) / endpoints.length;

    // Calculate average error rate across all endpoints
    ...
    if (slowestEndpoints.length > 0 && slowestEndpoints[0].averageResponseTime > 3000) {
      await this.createAlert({
        name: 'Slow Endpoint Detected',
        description: `Endpoint ${slowestEndpoints[0].endpoint} ${slowestEndpoints[0].method} has response time of ${slowestEndpoints[0].averageResponseTime.toFixed(2)}ms`,
        severity: 'HIGH',
        category: 'PERFORMANCE',
        source: 'performance_service',
        currentValue: slowestEndpoints[0].averageResponseTime,
        threshold: {
        ...
        metadata: {
          endpoint: slowestEndpoints[0].endpoint,
          method: slowestEndpoints[0].method
        }
      });
    }

    // Low cache hit rate alert (simulated based on total requests and endpoint performance)
    const totalRequests = endpoints.reduce((sum, ep) => sum + ep.totalRequests, 0);
    const lowCacheHitRate = 30; // Simulated low cache hit rate
    if (totalRequests > 200) {
      await this.createAlert({
        name: 'Low Cache Hit Rate',
        description: `Cache hit rate is ${lowCacheHitRate.toFixed(2)}%`,
        severity: 'MEDIUM',
        category: 'PERFORMANCE',
        source: 'performance_service',
        currentValue: lowCacheHitRate,
        threshold: {
    ...
    try {
      ...
      if (healthResult.status === 'UNHEALTHY') {
        await this.createAlert({
          ...
          metadata: {
            failedChecks: healthResult.checks.filter((c: any) => c.status === 'DOWN' || c.status === 'UNHEALTHY'),
          },
        });
        ...
        await this.createAlert({
          ...
          tags: ['availability', 'health'],
          metadata: {
            degradedChecks: healthResult.checks.filter((c: any) => c.status === 'DEGRADED'),
          },
        });
      }
    } catch (error) {
  ...
  async checkErrorAlerts(): Promise<void> {
    try {
      ...
      if (errorStats && errorStats.logsByLevel && errorStats.logsByLevel.fatal > 0) {
        await this.createAlert({
          name: 'Fatal Errors Detected',
          description: `${errorStats.logsByLevel.fatal} fatal errors in the last hour`,
          severity: 'CRITICAL',
          category: 'SYSTEM',
          source: 'logging_service',
          currentValue: errorStats.logsByLevel.fatal,
          threshold: {
            metric: 'fatal_errors',
            operator: 'gt',
            value: 0,
      ...
      if (errorStats && errorStats.logsByLevel && errorStats.logsByLevel.error > 10) {
        await this.createAlert({
          name: 'High Error Rate',
          description: `${errorStats.logsByLevel.error} errors in the last hour`,
          severity: 'HIGH',
          category: 'SYSTEM',
          source: 'logging_service',
          currentValue: errorStats.logsByLevel.error,
          threshold: {
            metric: 'error_count',
            operator: 'gt',
            value: 10,
      ...
      this.logger.error('Error alert check failed:', error);
    }
  }
  ...
  private async sendEmailNotification(_channel: NotificationChannel, message: NotificationMessage): Promise<void> {
    // In a real implementation, this would use an email service like SendGrid, SES, etc.
    this.enhancedLogger.info(`EMAIL notification sent to ${message.recipients.join(', ')}: ${message.subject}`, 'ALERT');
  }
  ...
  private async sendResolutionNotification(alert: Alert, channel: NotificationChannel): Promise<void> {
    const message = {
      alert: { ...alert, status: 'RESOLVED' as const },
      channel,
